...
const MultiSelect: React.FC<MultiSelectProps> = ({ 
    ...
    const handleGroupSelect = (groupOptions: string[]) => {
        ...
        if (allSelected) {
            ...
            const combinedArray = Array.from(new Set([...value, ...groupOptions]));
            ...
            {isOpen && (
                <div className="absolute z-50 w-full mt-1 bg-neutral-800 border border-neutral-700 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                    {Object.entries(groups).map(([groupName, groupOptions]) => (
                        <div key={groupName} className="p-2 border-b border-neutral-700">
                            <label className="flex items-center gap-2 px-2 py-1 hover:bg-neutral-700 rounded cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={groupOptions.every(option => value.includes(option))}
                                    onChange={() => handleGroupSelect(groupOptions)}
                                    className="rounded border-neutral-500"
                                />
                                <span className="text-neutral-200">{groupName}</span>
                            </label>
                        </div>
                    ))}
...
// Main Component
export const SidePanelControl: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
...
const [clusterVisibility, setClusterVisibility] = useState<VisibilityState>({ isActive: false });
    const [edgeVisibility, setEdgeVisibility] = useState<VisibilityState>({ isActive: false, mode: 'off' });
...
const resetEdgeOptions = () => {
    ...
    const handleOptionChange = (
        ...
        switch (optionType) {
            case 'highlight':
                setHighlightOptions((prev: CommonOptions) => ({ ...prev, [field]: value }));
                break;
            case 'cluster':
                setClusterOptions((prev: CommonOptions) => ({ ...prev, [field]: value }));
                break;
            case 'edge':
                setEdgeOptions((prev: ContextEdgeOptions) => {
                    if (field === 'visibility') {
                        ...
                        if (visibilityValue === 'on' || visibilityValue === 'hover' || visibilityValue === 'off') {
                            return {
                                ...prev,
            ...
            <div className="space-y-3">
                ...
                <FilterOption label="Article Contains the Broad Claim/s:">
                    <MultiSelect
                        ...
                        onChange={(value) => handleOptionChange(optionType, 'broadClaim', value.join(','))}
                ...
                <FilterOption label="Article Contains the Sub Claim/s:">
                    <MultiSelect
                        ...
                        onChange={(value) => handleOptionChange(optionType, 'subClaim', value.join(','))}
                ...
                <FilterOption label="Article is from Source/s:">
                    <MultiSelect
                        ...
                        onChange={(value) => handleOptionChange(optionType, 'source', value.join(','))}
